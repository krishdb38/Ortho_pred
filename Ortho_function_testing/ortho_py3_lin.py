...
def Get_Same_Species_Forward_Best_Hit(blastp_score): 
    ...
    del blastp_score_split[-1] # delete of [''] in the last index  ex) ['gi,gi,1,1','gi,gi,2,2','']
...
def Parallel_MCL(score_matrix):    
    ...
    while idempotent_matrix.sum() > infinitesimal_value: # > infinitesimal_value 
        ...
        divide_results = pool.map(Parallel_Matrix_Divide_Using_Numpy, zip(power_results, repeat(sum_matrix)))
        pool.close()
        pool.join()
