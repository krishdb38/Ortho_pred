...
"""
This is a program to detect ortholog between species.
...
def run_parallel_query(species_of_query, species_of_subject, query_v, parallel_num):
    ...
    global SELECTED_NUMBER, SELECTED_SPECIS_DIC
                ...
                if species_of_query == species_of_subject:
                    ...
                    for best_score_element in same_species_forward_best_score:
                        ...
                        if best_score_element[0] == best_score_element[1]:
                            with open(Score_file+SELECTED_SPECIS_DIC[species_of_query] \
                                ...
                                save_best_score = SELECTED_SPECIS_DIC[species_of_query]\
                                    + "_" + best_score_element[0]\
                                    .split(r"\s")[0] + " " + SELECTED_SPECIS_DIC[species_of_subject]\
                            ...
                            with open(Score_file+SELECTED_SPECIS_DIC[species_of_query]\
                                ...
                                best_score_save = SELECTED_SPECIS_DIC[species_of_query]\
                                    ...
                                    + "_"+best_score_element[1].split(
                                        r"\s")[0]+" "+best_score_element[2]+" "+best_score_element[3]+"\n"
                    ...
                    for blastp_score_split_list_element in blastp_score_split_list:
                        with open(Score_file+SELECTED_SPECIS_DIC[species_of_query]+"_"
                            ...
                            blastp_score_split_list_save = SELECTED_SPECIS_DIC[species_of_query]+"_"\
                                + blastp_score_split_list_element[0].split(r"\s")[0]\
                                + " "+SELECTED_SPECIS_DIC[species_of_subject]+"_"+blastp_score_split_list_element[1].split(r"\s")[0]\
                                + " " + \
                ...
                if k < i:  # gene ====> query 1->1 1->2 1->3 2->2 2->3  Forward checking will skip the same or less
                    ...
                    if cpu_count == 1:
                        ...
                        if query_v_len < cpu_count:
                            # "because the cpu_count will seprate list items from query_v file.
                            # In general this will not happened because length of query is always long"
                            blastp_time_start = time.time()
...
def backward_best_hit(args):
    ...
    print(bck_info + " between %s genome %s genome" % (
        SELECTED_SPECIS_DIC[species_of_query], SELECTED_SPECIS_DIC[species_of_subject]))
    ...
    # "bar = Bar("Searching : "+SELECTED_SPECIS_DIC[species_of_query]+"-"+SELECTED_SPECIS_DIC[species_of_subject], max = len(forward_best_hit_score_list))" #remove ""
    ...
    # +"_"+SELECTED_SPECIS_DIC[species_of_subject]+'_subtraction'+"_"+str(threshold_score), 'a') as subtraction :
    ...
    print(bck_info + " %s-%s took %.2f minutes" %
          (SELECTED_SPECIS_DIC[species_of_query], SELECTED_SPECIS_DIC[species_of_subject], rbh_time))
    return rbh_time
