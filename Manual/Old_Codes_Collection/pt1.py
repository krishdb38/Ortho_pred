...
class OneWayBlastp:
    ...
    def Oneway_Threshold_Best_Hit(self, mode):
        OWBP = OneWayBlastp()    
        process_list = []
    ...
    def Backward_Best_Hit(self, args):
...
def main(argv):
    ...
    global user_selected_number
    global Species
    global Blastp_data
    global blastp_matrix
    global Blastp
    global precalculated_data_list
    global used_precalculated_data_list
    global equal_BBH_data
    global equal_BBH_data_dic
    global unequal_BBH_data
    global results
    global second_equal_BBH_data
    global tasks
    ...
    if "1" in mode :            
        backward_best_hit_work_list = OWBP.Oneway_Threshold_Best_Hit(mode)    
        pool = multiprocessing.Pool(cpu_count)    
        results = pool.map(OWBP.Backward_Best_Hit,backward_best_hit_work_list)
        pool.close()
        pool.join()   
