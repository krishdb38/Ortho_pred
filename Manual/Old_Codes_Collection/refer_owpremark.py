...
blastp_group.add_argument('-b', '--blastp',
                          ...
                          dest='Blastp',
                          help='set the path of blastp to run the blast program. The default is "/usr/bin/blastp".')
blastp_group.add_argument('-F', '--scorefile',
                          action='store',
                          default='./score_file/',
                          dest='Score_file',
                          help='The outputs of the blast program are saved in the score_file directory. The default path is "./score_file/".')
...
mcl_group.add_argument('-o', '--out',
                             ...
                             default='./cluster_out',
                             dest='Cluster_out',
                       help='set the path of the ortholog cluster file, log and error_warning file. The default path is "./cluster_out".')
...
class OneWayBlastp:
        ...
        if queryV_len % cpu_count == 0:  # perfect division
            for i in range(cpu_count):
                ...
                if parallel_query_start == 0:
                    parallel_query.append(
                        queryV[parallel_query_start:i*query_division_value])
                    parallel_query_start += 1
                else:
                    parallel_query.append(
                        queryV[parallel_query_start*query_division_value:i*query_division_value])
                    parallel_query_start += 1
            ...
            for i in range(cpu_count):
                ...
                if parallel_query_start == 0:
                    ...
                    parallel_query.append(
                        queryV[parallel_query_start*query_division_value:])
                    parallel_query_start += 1
    ...
    def RunParallelQuery(self, species_of_query, species_of_subject, queryV, parallel_num):
        ...
        Save the files which are oneway_threshold_best_hit, blastp_score_split_list and raw_blastp_score (optional) by each species. """
        bar = Bar('Processing '+str(parallel_num),
                  max=len(queryV))  # progressing bar setting
        OWBP = OneWayBlastp()
        for j in queryV:
            bar.next()  # progressing bar print
            OWBP.WriteQuery(j, parallel_num)
            blastp_score = OWBP.RunBlast(
                selected_species_dic[species_of_subject], parallel_num)
            if blastp_score != '':  # Check whether blastp_score has the value

                best_score, blastp_score_split_list = OWBP.GetForwardBestHit(
                    blastp_score)

                # ex) AAE == AAE. It will save best_score without reversing RunBlast.
                if species_of_query == species_of_subject:
                    same_species_forward_best_score = OWBP.Get_Same_Species_Forward_Best_Hit(
                        blastp_score)
                    for best_score_element in same_species_forward_best_score:
                        ...
                        if best_score_element[0] == best_score_element[1]:
                            ...
                            with open(Score_file+selected_species_dic[species_of_query]+"_"+selected_species_dic[species_of_subject]+"_oneway_threshold_best_hit_S"+str(threshold_score), "a") as oneway_threshold_best_hit:
                                ...
                                save_best_score = selected_species_dic[species_of_query]+"_"+best_score_element[0].split("|")[1]+" "+selected_species_dic[species_of_subject]+"_"+best_score_element[1].split(
                                    "|")[1]+" "+best_score_element[2]+"\n"  # best_score_element[0].split("|") ==> ['gi', '15642790', 'ref', 'NP_227831.1', '']
                    ...
                    for best_score_element in best_score:
                        with open(Score_file+selected_species_dic[species_of_query]+"_"+selected_species_dic[species_of_subject]+"_"+"best_score_S"+str(threshold_score)+"_"+str(parallel_num), "a") as save_best_hit:
                            best_score_save = selected_species_dic[species_of_query]+"_"+best_score_element[0].split(
                                "|")[1]+" "+selected_species_dic[species_of_subject]+"_"+best_score_element[1].split("|")[1]+" "+best_score_element[2]+" "+best_score_element[3]+"\n"
                    ...
                    for blastp_score_split_list_element in blastp_score_split_list:
                        with open(Score_file+selected_species_dic[species_of_query]+"_"+selected_species_dic[species_of_subject]+"_"+"blastp_score_split_list_S"+str(threshold_score)+"_"+str(parallel_num), "a") as save_blastp_score_split_list:
                            blastp_score_split_list_save = selected_species_dic[species_of_query]+"_"+blastp_score_split_list_element[0].split(
                                "|")[1]+" "+selected_species_dic[species_of_subject]+"_"+blastp_score_split_list_element[1].split("|")[1]+" "+blastp_score_split_list_element[2]+" "+blastp_score_split_list_element[3]+"\n"
                ...
                if k < i:  # gene ====> query 1->1 1->2 1->3 2->2 2->3
                    ...
                    if cpu_count == 1:
                        ...
                        if queryV_len < cpu_count:
                            ...
                            parallel_query = OWBP.DivisionParallelQuery(
                                queryV, 1, queryV_len, queryV_len)
                            for m in range(queryV_len):
                                process = multiprocessing.Process(
                                    target=OWBP.RunParallelQuery, args=(i, k, parallel_query[m], m+1))
...
def Backward_Best_Hit(args):
    ...
    bar = Bar("Searching : "+selected_species_dic[species_of_query]+"-" +
              selected_species_dic[species_of_subject], max=len(forward_best_hit_score_list))
                        ...
                        with open(Cluster_out+"_geneID_S"+str(threshold_score)+"_"+str(inflation_factor), "a") as ortholog_list_save:
                            ...
                            while not ortholog.empty():
                                ...
                                try:
                                    ...
                                    get_ortholog_split = get_ortholog.split(
                                        '_')
                                    gene_id_queue.put(
...
def Parallel_MCL(score_matrix):
    ...
    while idempotent_value > infinitesimal_value:  # > infinitesimal_value
        ...
        multiplication_results = pool.map(
            Parallel_Matrix_Multiplication_Using_Numpy, izip(score_matrix, repeat(score_matrix)))
        pool.close()
        pool.join()
        ...
        pool = multiprocessing.Pool(cpu_count)
        divide_results = pool.map(Parallel_Matrix_Divide_Using_Numpy, izip(
        ...
        # identify whether inflation_matrix is idempotent matrix or not.
        idempotent_value = abs(numpy.sum(score_matrix) -
                               numpy.sum(multiplication_results))
...
def Del_File(path, file):
    del_file = subprocess.Popen(
        ["rm "+path+file], stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
...
def main(argv):
    ...
    if not sys.argv[1:]:
        ...
        selected_number = raw_input(
            ">> Select an genomes to detect orthologs(e.g. 1 2 3 4 5 or 1-5) : ")
        ...
        user_selected_number = [
            backward_selected_species_dic[ele] for ele in genomes]
    ...
    if "3" in mode:
        ...
        mcl_time_log = float(
            (finish_time_clustering - start_time_clustering)/60)
        ...
        if "3" in mode:
            with open(Log_file_name, 'a') as log:
                ...
                log.write("Remark program took " +
                          str(remark_time_log) + "minutes\n")
