...
parser = argparse.ArgumentParser(description = 'The owPReMark is a program to detect orthologs between the protein sequences from different genomes using one way blast best hits for clustering orthologs efficiently.', 
...
essential_group.add_argument('-M','--mode', 
                             ...
                             dest='mode',
                             choices=['1','2','3'],
                             help="1 : BLASTP. 2 : BLASTP with precalculated data. 3 : MCL. select just one of four modes : 1 or 2 or 1 3 or 2 3")
essential_group.add_argument('-g','--genomes',
                             nargs='+',
                             dest='genomes', 
                             help="select genomes, from which orthologs are detected.")
...
blastp_group.add_argument('+r', 
                             ...
                             help='save the raw score of blastp to the blastp_data dirctory.')
blastp_group.add_argument('-t', '--threshold', 
                             action='store', 
                             default=0, 
                             type=int,
                             dest='threshold_score', 
                             help='set the threshold score. The threshold score has an allowable range to detect orthologs.'+
                             'If the score difference between the backward best hit score and the forward best hit score is less than or equal to the threshold score, it is considered the ortholog pairs.'+
                             'We can detect orthologs using the "one-way threshold blast best hits" named by us. The default value is "5". It is required that the difference between the backward best hit score and the forward best hit score should be less than or equal to the default value.')
...
blastp_group.add_argument('-s','--species', 
                             ...
                             default = './species/', 
                             dest='Species', 
                             help='set the path of the directory containing species selected. The default is  "./species/".')
blastp_group.add_argument('-b','--blastp', 
                             action='store', 
                             default ='/usr/bin/blastp', 
                             dest='Blastp', 
                             help='set the path of blastp file to run the blastp program. The default is "/usr/bin/blastp".')
blastp_group.add_argument('-B','--blastp_data', 
                             action='store', 
                             default='./blastp_data/', 
                             dest='Blastp_data', 
                             help='set the path of precalculated blastp data. If you want to use the blastp data calculated previously, you should move the data of the Score_file directory to the blastp_data directory. The default path is "./blastp_data/".')
...
mcl_group.add_argument('-l', '--loop', 
                             ...
                             type=int,
                             dest='infinite_loop', 
                             help='prevent the infinite loop of MCL algorithm. The default value is "100".')
mcl_group.add_argument('-o','--out', 
                             action='store', 
                             default='./cluster_out', 
                             dest='Cluster_out', 
                             help='set the path of the ortholog cluster file, log and error_warning file. The default path and name is "./cluster_out".')
    ...
    def RunParallelQuery(self, species_of_query, species_of_subject,queryV, parallel_num):    
        ...
        for j in queryV:
                ...
                if species_of_query == species_of_subject: # ex) AAE == AAE. It will save best_score without reversing RunBlast. 
                    ...
                    for best_score_element in same_species_forward_best_score:
                        if best_score_element[0] == best_score_element[1]:  #  ex) [A1 of AAE, A1 of AAE, 30]                                           
                            ...
                            with open(Blastp_data+selected_species_dic[species_of_query]+"_"+selected_species_dic[species_of_subject]+"_oneway_threshold_best_hit_S"+str(threshold_score), "a") as oneway_threshold_best_hit:
                            ...
                            with open(Blastp_data+selected_species_dic[species_of_query]+"_"+selected_species_dic[species_of_subject]+"_second_oneway_threshold_best_hit_S"+str(threshold_score), "a") as second_oneway_threshold_best_hit:
                    ...
                    for best_score_element in best_score:
                        with open(Blastp_data+selected_species_dic[species_of_query]+"_"+selected_species_dic[species_of_subject]+"_"+"best_score_S"+str(threshold_score)+"_"+str(parallel_num), "a") as save_best_hit:
                    ...
                    for blastp_score_split_list_element in blastp_score_split_list:
                        with open(Blastp_data+selected_species_dic[species_of_query]+"_"+selected_species_dic[species_of_subject]+"_"+"blastp_score_split_list_S"+str(threshold_score)+"_"+str(parallel_num), "a") as save_blastp_score_split_list:                            
            ...
            if save_raw_blastp_score :
                 with open(Blastp_data+selected_species_dic[species_of_query]+"_"+selected_species_dic[species_of_subject]+"_S"+str(threshold_score)+"_"+str(parallel_num), "a") as save_blastp:
...
def Backward_Best_Hit(args):
    ...
    if queryV_len < cpu_count : #If the number of queryV_len is less than cpu_count, the cpu_count is changed to queryV_len.
        for parallel_num in range(queryV_len):
            parallel_num += 1                                  
            with open(Blastp_data+selected_species_dic[species_of_query]+"_"+selected_species_dic[species_of_subject]+"_"+"best_score_S"+str(threshold_score)+"_"+str(parallel_num), "r") as best_hit_score:
        ...
        #            print split_each_line
            ...
            with open(Blastp_data+selected_species_dic[species_of_query]+"_"+selected_species_dic[species_of_subject]+"_"+"blastp_score_split_list_S"+str(threshold_score)+"_"+str(parallel_num), 'r') as blastp_score :
...
#            subtraction.write(str(save_data)+"\n")
        ...
        if int(backward_best_score[2]) - int(forward_best_hit_score_element[2]) <= threshold_score :           
            with open(Blastp_data+selected_species_dic[species_of_query]+"_"+selected_species_dic[species_of_subject]+"_oneway_threshold_best_hit_S"+str(threshold_score), "a") as other_oneway_threshold_best_hit:
...
class MclDetect:
    def Generating_Matrix_Genepairs(self, element_set, bar):  
        """ Generate the matrix comprising gene pairs detected by oneway threshold blast best hits. """
        ...
        if len(row_data)*len(col_data) > 4 :  # If the elements of row and col is less than 2, it is excluded.
            if len(row_data) > 1000 and cpu_count > 1 : #The big size of matrix(bigger than 1000 X 1000) will be computed by Parallel_Matrix_Multiplication_Using_Numpy function.            
                idempotent_matrix = Parallel_MCL(score_matrix)
            else :            
                idempotent_matrix = MD.MCL(score_matrix)
            MD.Detecting(row_data, col_data, idempotent_matrix)    


    def MCL(self, inflation_matrix):    
        count = 0
        infinitesimal_value = 10**-10
        check_matrix = numpy.matlib.ones((2,2))    
        while check_matrix.sum() > infinitesimal_value: # > infinitesimal_value 
            MCL_time_start = time.time()        
            expansion_matrix = inflation_matrix ** 2      
            inflation_matrix = numpy.power(expansion_matrix, inflation_factor)
            inflation_matrix_sum =  inflation_matrix.sum(axis = 0)
            inflation_matrix = numpy.divide(inflation_matrix, inflation_matrix_sum) # create a inflation_matrix        
            check_matrix =abs(inflation_matrix - expansion_matrix) # identify whether inflation_matrix is idempotent matrix or not.        
            count += 1
            if count > infinite_loop : # prevent the infinite loop of MCL algorithm.       
                break     
            MCL_time_finish = time.time()
            if verbose :
                print " MCL time : %f, count : %d, matrix size : %d * %d" % ((MCL_time_finish - MCL_time_start)/60, count, inflation_matrix[0].size, inflation_matrix[0].size)            
        return inflation_matrix

    def Detecting(self, row_data, col_data, idempotent_matrix):
        ...
        for i in range(len(row_data)):
            ortholog_list = []
            ortholog_sum = 0
            ortholog = Queue.Queue() # Queue which puts orthologs.
            gene_id_queue = Queue.Queue() # Queue which puts each ortholog with a changed gene ID 
            for j in range(len(col_data)):
                if 0.1 <= idempotent_matrix[i,j]:            
                    ortholog.put(col_data[j])
                    ortholog_list.append(col_data[j])

            if ortholog.qsize() >= 3: # If the ortholog queue includes the elements of orthologs larger than 2, then output the orthologs.
                        ...
                        with open(Cluster_out+"_geneID_S"+str(threshold_score)+"_"+str(inflation_factor), "a") as ortholog_list_save:
                            ...
                            while not ortholog.empty():
                                ...
                                try:
                                    ...
                                    gene_id_queue.put(get_ortholog)  # If the gene_id_dic does not have get_ortholog, it will print the original ID(get_ortholog).
...
def Parallel_MCL(score_matrix):    
    ...
    while check_value > infinitesimal_value: # > infinitesimal_value 
        MCL_time_start = time.time()        
        ...
        check_value = abs(numpy.sum(score_matrix) - numpy.sum(multiplication_results)) # identify whether inflation_matrix is idempotent matrix or not.
...
def main(argv):
    ...
    global verbose, cluster_count, gene_id_dic, ortholog_count, cpu_count, save_raw_blastp_score
    ...
    if not sys.argv[1:]:    
        ...
        selected_number= raw_input(">> Select genomes to detect orthologs(e.g. 1 2 3 4 5 or 1-5) : ")
    ...
    if "3" in mode :
        ...
        ##########################################################################################
        #generate matrix and calculate the matrix using MCL algorithm."""    
        print "\n>>>> Start MCL and detecting orthologs <<<<"    
        equal_BBH_data = []
        unequal_BBH_data = []
        equal_BBH_data_dic ={}
        ...
        if "1" in mode :
            for i in user_selected_number:
                for k in user_selected_number:       
                    if k < i :
                        ...
                        OWBP.Read_Equal_BBH(Blastp_data+selected_species_dic[i]+"_"+selected_species_dic[k])
                    elif i != k :                              
                        OWBP.Read_Unequal_BBH(Blastp_data+selected_species_dic[i]+"_"+selected_species_dic[k])
            ...
            for new_data in new_calculated_data_list :
                ...
                if first == second :
                    OWBP.Read_Equal_BBH(Blastp_data+new_data)
                elif first != second :
                    OWBP.Read_Unequal_BBH(Blastp_data+new_data)          
        ...
        for data in matched_BBH_data :
            MD.Generating_Matrix_Genepairs(data, bar) 
        bar.finish()
        ...
        print "MCL and detecting orthologs took %.2f minutes" % mcl_time_log
        ...
        if "3" in mode :
            with open(Log_file_name, 'a') as log:
                ...
                log.write("MCL and detecting orthologs took "+str(mcl_time_log)+" minutes\n")
